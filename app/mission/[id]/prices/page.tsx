...
import { basicOffers, type BasicOffer } from "@/lib/pipelineOffers";

function sortedByPrice(offers: BasicOffer[]): BasicOffer[] {
  const quoted = offers.filter((o) => o.price !== null);
  const waiting = offers.filter((o) => o.price === null);
  quoted.sort((a, b) => (a.price as number) - (b.price as number));
...
export default function PricesPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const offers = sortedByPrice(basicOffers(mission));
  const quoted = offers.filter((o) => o.price !== null);
  const waiting = offers.filter((o) => o.price === null);
  const pending = pendingId ? offers.find((o) => o.quoteId === pendingId) ?? null : null;

  const confirmSelection = async (offer: BasicOffer) => {
          ...
          {waiting.length > 0 && (
            <>
              <div className="flex items-center gap-3 py-2">
                ...
                <span className="text-xs font-medium text-gray-400">Still calling</span>
                <div className="h-px flex-1 bg-gray-200" />
              </div>
              {waiting.map((offer) => (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-6">
          <div className="w-full max-w-sm rounded-3xl bg-white p-6 shadow-xl">
            ...
            <p className="mt-2 text-sm text-gray-500">
              Are you sure you want to select <span className="font-semibold text-black">{pending.name}</span> at ${pending.price}?
            </p>
