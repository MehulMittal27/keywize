...
export default function PricesPage({ params }: { params: Promise<{ id: string }> }) {
  const { id } = use(params);
  const [results, setResults] = useState<LocksmithPriceResult[]>([]);
  ...
  // mismatch from date-based labels, which was forcing React into a full
  // remount and leaving stale, unresponsive event handlers behind.
  ...
  useEffect(() => {
    ...
    async function fetchPrices() {
      // Simulate agents running in parallel (1.5s visual delay)
  ...
  const quoted = results.filter((r) => r.basicPrice !== null);
  const unquoted = results.filter((r) => r.basicPrice === null);
  const cheapest = quoted[0] ?? null;
    ...
    <div className="min-h-screen bg-[#fbfaf7] text-[#111111] font-sans pb-24">
      ...
      <nav className="flex items-center justify-between px-8 py-5 max-w-4xl mx-auto border-b border-black/5">
        ...
        <div className="flex items-center gap-4">
          ...
          <span className="text-sm font-medium text-gray-400">Price Discovery</span>
        </div>
      </nav>
      ...
      <main className="max-w-4xl mx-auto mt-10 px-6">
        ...
        <div className="mb-8">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#30a985]/10 text-[#30a985] text-xs font-bold uppercase tracking-wide border border-[#30a985]/20 mb-4">
            ...
            {agentsRunning ? "Agents calling..." : `${quoted.length} quotes collected`}
          </div>
          <h1 className="text-4xl font-serif tracking-tight mb-2">
            Price Discovery
          </h1>
          <p className="text-gray-500 text-base">
            10 agents called locksmiths concurrently. Results sorted by basic price, lowest first.
          </p>
        </div>
        ...
        {!loading && results.length > 0 && (
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-12 gap-4 px-5 py-2 text-xs font-semibold text-gray-400 uppercase tracking-wider">
              ...
              <div className="col-span-2 text-right">Basic Price</div>
            </div>
                  ...
                  {i === 0 && !confirmedVendor && (
                    <div className="px-5 pb-3">
                      <span className="inline-flex items-center gap-1 text-xs font-semibold text-[#30a985] bg-[#30a985]/10 px-2 py-0.5 rounded-full">
                        Lowest price — candidate for negotiation
                      </span>
                    </div>
                  )}
        ...
        {!loading && confirmedResult && (
          <div className="mt-8 bg-[#30a985] text-white rounded-3xl p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            ...
            <div className="text-right">
              <div className="text-4xl font-serif font-bold">${confirmedResult.basicPrice}</div>
              <div className="text-xs text-white/70 mt-1">Confirmed · dispatch will be notified</div>
            </div>
          </div>
        )}

        {!loading && !confirmedResult && cheapest && (
          <div className="mt-8 bg-[#111111] text-white rounded-3xl p-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              <div className="text-xs font-bold uppercase tracking-wider text-gray-400 mb-1">Best starting point</div>
              <div className="text-xl font-bold">{cheapest.name}</div>
              <div className="text-sm text-gray-400 mt-0.5">{cheapest.address}</div>
            </div>
            <div className="text-right">
              <div className="text-4xl font-serif font-bold text-[#30a985]">${cheapest.basicPrice}</div>
              <div className="text-xs text-gray-400 mt-1">Basic quote · ready to negotiate</div>
            </div>
          </div>
        )}
      ...
      {pendingResult && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 px-6">
          <div className="bg-white rounded-3xl shadow-xl max-w-sm w-full p-6">
            <h3 className="text-lg font-serif font-semibold mb-2">Skip negotiation?</h3>
            <p className="text-sm text-gray-500 mb-5">
              Are you sure you don&rsquo;t want to wait for negotiation? Keywize may still be able to get{" "}
              <span className="font-semibold text-[#111111]">{pendingResult.name}</span> to lower their ${pendingResult.basicPrice} quote.
            </p>
