...
export function VoiceTrustBadge({
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-medium ${style.bg}`}
    >
      <span className={`h-2 w-2 rounded-full ${style.dot} shrink-0`} />
      {compact ? `VoiceTrust: ${level}` : style.label}
...
export default VoiceTrustBadge;
